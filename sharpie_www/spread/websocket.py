...
class Consumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        actions = self.agent[self.room_name].agent_step(next_states, rewards, terminated)
        ...
        if all(a == 0 for a in terminated):
            self.agent[self.room_name].model.save(self.static_folder[self.room_name]+'agent.keras')
