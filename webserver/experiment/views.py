...
from .settings import ConfigForm, inputsListened, experiment_name
...
def config_(request):
    ...
    return render(request, "experiment/config.html", {'form': form, 'saved': saved, 'experiment_name': experiment_name})
...
def run_(request):
    ...
    if not saved:
        return redirect("/experiment/config")

    room_name = request.session['room_name']
    return render(request, "experiment/run.html", {"room_name": room_name, 'ws_setting': WS_SETTING, 'inputsListened': inputsListened, 'experiment_name': experiment_name})