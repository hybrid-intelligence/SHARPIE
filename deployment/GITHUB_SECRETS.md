...
For additional security, consider using environment-specific secrets:
...
Update the workflow to use environment:
...
You can also scope secrets to specific environments:

1. Go to **Settings** → **Environments** → **production**
2. Add environment secrets
3. These override repository secrets for that environment

This allows different secrets for staging vs production environments.

## Troubleshooting

### "Error: Permission denied (publickey)"

- Verify `DEPLOY_KEY` contains the correct private key
- Check the public key is in `~/.ssh/authorized_keys` on the server
- Verify `PRODUCTION_USER` matches the server user

### "Error: Connection refused"

- Verify `PRODUCTION_HOST` is correct
- Check the server is accessible (firewall, network)
- Verify SSH port (default 22) is open

### "Error: Host key verification failed"

Add this step before the SSH action to accept the host key:
...
- Verify `PRODUCTION_URL` is correct and accessible
- Check if the application is running on the server
- Verify nginx is properly configured
- Ensure the health endpoint exists or modify the workflow
